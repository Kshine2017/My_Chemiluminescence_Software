#include "calibration_twowidget.h"
#include <QDebug>
#include <QHeaderView>
#include <QAbstractItemDelegate>
calibration_twoWidget::calibration_twoWidget(QWidget *parent) : QWidget(parent)
{
    ...
    /***工作队列和超时定时器****/
    queue_WorkQueue = new Queue(this);
    ...
    connect(questionBtn,SIGNAL(clicked()),this,SLOT(questionBtnSlot()));
    ...
    connect(this->queryTimer,SIGNAL(timeout()),this,SLOT(queue_queryTimerOverSlot()));
 ...
 * @brief 窗口标题头初始化
 */
void calibration_twoWidget::titleInit()
{
 ...
 * @brief 模型视图初始化
 */
void calibration_twoWidget::tableViewInit()
{
...
//    }
    ...
    /*创建代理*/
    progressBarDelegate = new calibration_ProgressBarDelegate;
    ...
    model->setHeaderData(1,Qt::Horizontal,QObject::tr("质控品条码"));
...
//    tableView->setEditTriggers(QAbstractItemView::NoEditTriggers); //QAbstractItemView::EditingState
    tableView->setEditTriggers(QAbstractItemView::AllEditTriggers); //允许所有的编辑操作，主要是为了 combobox可以单击
//    (QTableView)(tableView->childAt(2,2)).setEditTriggers(QAbstractItemView::NoEditTriggers);
    //隐藏表格线
 ...
 * @brief 操作按钮初始化
 */
void calibration_twoWidget::PushBtnInit()
{
 ...
 * @return 当前试剂运行数据
 */
QString calibration_twoWidget::getRunInfo(QString &serical)
{
...
void calibration_twoWidget::QCustomplotInit()
{
 ...
 * @brief 结束放置按钮槽
 */
void calibration_twoWidget::finishPlaceBtnSlot()
{
    for(int i = 0; i < 12 ; i++){
    ...
    for(it = RunData->begin();it != RunData->end();it++){
        ...
        queue_WorkQueue->enqueue("05,06,ff,ff,00,00"+ QString(",")+ QString::number(postion,10)+QString(",")+ it.value()); //将当前试剂船运动数据加入工作队列
    }
 ...
 * @brief 返回按钮槽
 */
void calibration_twoWidget::returnBtnSlot()
{
    qDebug()<< QString(tr("返回按钮"));
}

/**
 * @brief 下一步按钮槽
 */
void calibration_twoWidget::nextBtnSlot()
{
    qDebug()<< QString(tr("下一步按钮"));
    emit measurementFinshSignal(3);
    //todo
}
 ...
 * @brief 问号按钮槽
 */
void calibration_twoWidget::questionBtnSlot()
{
    qDebug()<< QString(tr("问号按钮"));
}


/**
 * @brief calibration_okBtnDelegate::显示曲线图信号槽
 * @param index返回第几行，从0开始
 */
void calibration_twoWidget::Display_btnSlot(const QModelIndex &index)
{
    int row = index.row();
...
/**
 * @brief calibration_twoWidget::质控品询问空闲超时定时器超时信号槽
 */
void calibration_twoWidget::queue_queryTimerOverSlot()
{
    if( !queue_WorkQueue->isEmpty() ){
       serialPort::mySend_Data("05,05,ff,ff,00,00");
    }
}
