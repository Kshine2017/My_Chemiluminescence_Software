...
MyDialogQuery::MyDialogQuery(QWidget *parent, WF_enum WinFlag) : QDialog(parent)
{
    setWindowModality(Qt::ApplicationModal);
    this->setWindowTitle(tr("查询信息"));
...
void MyDialogQuery::btn_cancelSlot()
{
    this->close();
}

void MyDialogQuery::btn_addSlot()
{
    ...
    if(Flag_Window==ORDINARYUSER)
        b= op.selectData_OrdinaryUser(name,passward,truename,department,hospital,phone_num,email,model);
    if(Flag_Window==ADMINUSER)
    {
        b= op.selectData_AdminUser(name,passward,truename,department,hospital,phone_num,email,model);
    }
