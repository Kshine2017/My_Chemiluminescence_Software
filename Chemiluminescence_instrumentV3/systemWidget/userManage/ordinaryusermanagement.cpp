...
#include "ordinaryusermanagement.h"
#include <QDebug>
#include <QHeaderView>
...
/**
 * @brief OrdinaryUserManagement::OrdinaryUserManagement
 * @param parent
 * 普通用户管理界面
 */


OrdinaryUserManagement::OrdinaryUserManagement(QWidget *parent) : QWidget(parent)
{
    ...
    model->setQuery("select usrname,pwd,name,department,hospital,phone_numeber,email from usrinfo;");
...
void OrdinaryUserManagement::btn_addSlot()
{
    MyDialogAdd *myDialog = new MyDialogAdd(this,ORDINARYUSER);
    connect(myDialog,SIGNAL(siganl_please_fresh_table()),this,SLOT(fresh_table()));
    myDialog->show();
}

void OrdinaryUserManagement::btn_updateSlot()
{
    if(chooseFlag==-1)
    {
    ...
    MyDialogUpdate *myDialog = new MyDialogUpdate(this,str0,str1,str2,str3,str4,str5,str6,ORDINARYUSER);
    connect(myDialog,SIGNAL(siganl_please_fresh_table()),this,SLOT(fresh_table()));
    myDialog->show();

}

void OrdinaryUserManagement::btn_deleteSlot()
{
    ...
    else
    {
        ...
        myMessgeBox *messge  = new myMessgeBox(myMessgeBox::MY_MESSAGE_WARNING,tr("提示"),tr("请选择需要删除的用户！"),this);
        if(messge->exec() == QDialog::Accepted){   //模式对话框
...
void OrdinaryUserManagement::btn_querySlot()
{
    myDialog_query = new MyDialogQuery(this,ORDINARYUSER);
    connect(myDialog_query,SIGNAL(siganl_please_fresh_table()),this,SLOT(fresh_table_select()));
    myDialog_query->show();
}

void OrdinaryUserManagement::fresh_table()
{
    qDebug()<<"刷新表格";
    model->setQuery("select usrname,pwd,name,department,hospital,phone_numeber,email from usrinfo;");
...
void OrdinaryUserManagement::fresh_table_select()
{
    tableView->setModel(&(myDialog_query->model));
}

void OrdinaryUserManagement::slot_doubleclick_tableview(const QModelIndex &index)
{
